...
// Νέο καθεστώς (Κατ. Γ'): Δεν υπάρχουν διοικητικοί βαθμοί.
// Για όλα τα κλιμάκια: Βαθμός = Φρουρός.
const FIXED_RANK = "Φρουρός";

// Κλιμάκιο: με βάση τα όρια του πίνακα Κατ. Γ' (0-3, 3-5, 5-7, ... , 39-41)
...
// Πίνακας κλιμακίων: Βασικός μισθός (Κατ. Γ')
...
function specialAllowance(_grade3, status) {
  // Επίδομα ιδ. συνθηκών: ΜΟΝΟ ανά οικογενειακή κατάσταση (όπως συμφωνήθηκε)
  const map = {
    'Άγαμος': 155,
    'Έγγαμος': 225,
    'Έγγαμος με τέκνα': 285
  };
  return map[status] ?? 0;
}
...
function calc() {
  ...
  // Σταθερός βαθμός
  const adminEl = document.getElementById('adminGradeOut');
  if(adminEl) adminEl.textContent = FIXED_RANK;
  document.getElementById('klimOut').textContent = String(klim);
  const klimInlineEl = document.getElementById('klimInline');
  if(klimInlineEl) klimInlineEl.textContent = `(Κλιμάκιο ${klim})`;
  ...
  const special = specialAllowance(FIXED_RANK, status);
...
function printPayslip(){
  ...
  const rowsE = [
    ['Βασικός μισθός', document.getElementById('oBase').textContent],    ['Οικογενειακή παροχή', document.getElementById('oFam').textContent],
  ...
  function renderCal(){
    ...
    for(let r=0;r<6;r++){
      ...
      for(let c=0;c<7;c++){
        ...
        let badgesHtml = "";
        if(two.length===1){
          badgesHtml = `<div class="badgeSingle"><div class="badge calBadge ${two[0]}">${labelOf(two[0])}</div></div>`;
        } else if(two.length===2){
          // Διπλοβάρδια: ΠΡ + ΝΥ (ή ΝΥ + ΠΡ) στην ίδια μέρα, μισό-μισό.
          badgesHtml = `<div class="badgeSplit"><div class="badgeHalf calBadge ${two[0]}">${labelOf(two[0])}</div><div class="badgeHalf calBadge ${two[1]}">${labelOf(two[1])}</div></div>`;
        }
